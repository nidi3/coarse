 ...
 * @param keepSelectors css selectors for all elements that should NOT be made rough
 * @param {Object} [options={}] Global configuration options for rough
 * @returns {string} The converted svg as a string
 */

const coarse = (input, keepSelectors = [], options = {}) => {
  ...
  const keep = svg.querySelectorAll(keepSelectors.join(','));

  // Loop through all child elements
  for (let i = 0; i < children.length; i += 1) {
    const original = children[i];
    if (!keep.includes(original)) {
      const params = [];
      let shapeType;
